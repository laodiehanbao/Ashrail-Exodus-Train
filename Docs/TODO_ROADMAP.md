...
- 接入 Cocos Creator prefab/node 绑定：由真实 Creator 组件实现 `src/presentation/ui/cocos` 的绑定接口，只渲染 UI state、转发 `ui_request_*`，不得承载业务规则。
...
- 第一阶段 UI 已完成逻辑测试驱动的 TypeScript state/request 层、Cocos-ready Presenter、无 `cc` node binding manifest、manifest-to-binding factory、P0 Cocos UI runtime 和 app request router；下一步需要真实 Cocos Creator prefab/node 组件绑定。
